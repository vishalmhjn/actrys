...
def run_simulation(path_trips, 
    ...
    if routing_how == "reroute":
        ...
        rerouting_adaptation_steps = str(int(kwargs.get("rerouting_adaptation_steps", "180")))
        ...
        sim_string = "sumo"+\
                            " -n "+ PATH_NETWORK+\
                            " -r " + path_trips+\
                            " --additional-files " + path_temp_additional +\
                            " -b " + str(TOD[0]*3600) +\
                            " -e " + str((1+TOD[-1])*3600)+\
                            " --device.rerouting.probability "+rerouting_prob+\
                            " --device.rerouting.threads "+ routing_threads+\
                            " --device.rerouting.period "+ rerouting_period+\
                            " --device.rerouting.synchronize --no-step-log"+\
                            " --device.rerouting.adaptation-interval "+ rerouting_adaptation+\
                            " --device.rerouting.adaptation-steps " +rerouting_adaptation_steps +\
                            " --meso-tls-flow-penalty " + meso_tls_flow_penalty +\
                            " --meso-junction-control true" + \
                            " --meso-tls-penalty " + tls_tt_penalty +\
                            " --meso-minor-penalty " + meso_minor_penalty +\
                            " --weights.priority-factor " + priority_factor
        ...
        if evaluation_run==True:
            sim_string +=   " --summary "+ "../../"+SCENARIO+"/"+temp_scenario_name+"/summary"+\
                            " --tripinfo-output "+ "../../"+SCENARIO+"/"+temp_scenario_name+"/trips_output"+\
                            " --vehroute-output "+ "../../"+SCENARIO+"/"+temp_scenario_name+"/routes.rou.xml"	
        # print(sim_string)
