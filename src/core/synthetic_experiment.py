...
def create_dummy_OD(num_taz, 
    ...
    y = np.array([1, 1, 0.5, 0.5, 1, 1, 2, 2.5, 3, 3.5, 3, 2.5, 2, 2, 1.5, 1.5, 2, 2.5, 3, 2.5, 2.5, 2, 2, 1]).reshape(-1,1)
    ...
    print("===== Using "+od_distribution+" distribution for creating OD vector ======")
    ...
    for i in range(0, num_intervals):
        temporal_noise = np.random.normal(0, upper_limit_temporal_noise, num_taz*num_taz)
...
def create_assignment(OD,
    ...
    for k in range(0, intervals):
        ...
        sample_list_od = list(range((k-num_correlated_intervals) * num_od_pairs,\
        ...
        for i in range(0, num_detectors):		
            ...
            if k==0:
                ...
                weights_correlated_od = np.random.uniform(min_sensor_correlation, 
                                                        max_sensor_correlation, 
                                                        num_correlated_od)
                X[list(indices_correlated_od),k*num_detectors+i]  = weights_correlated_od
                ...
                if go_random:
                    ...
                    for l in range(k-num_correlated_intervals, k+1):
                        ...
                        X[list(indices_correlated_od + l*num_od_pairs),k*num_detectors+i]  = X[list(indices_correlated_od),i]
...
def link_speeds(q, interval, traffic_regime, max_flow=7000, max_speed=120):
    ...
    flow = q/interval
    v = [(-k2 + np.sqrt(k2**2 + 4*flow*k1))/(2*k1), (-k2 - np.sqrt(k2**2 + 4*flow*k1))/(2*k1)][traffic_regime]
    # print(v)
    v = np.nan_to_num(v, 0)
    v = np.where(v<0, 0, v)
    return v
...
def synthetic_simulation(OD_long, W, t_state, interval, num_detectors):
    ...
    speed = np.zeros((reshape_counts.shape[0], reshape_counts.shape[1]), dtype="float32")
...
def synthetic_scenario_orchestrator(num_od, 
                            TOD_START, 
                            TOD_END, 
                            num_detectors,
                            time_interval,
    ...
    W = create_assignment(OD, num_detectors,temporal_influence, network_correlation=network_correlation)
    ...
    speed[congestion, :] = vectorize_speeds(reshape_counts[congestion,:], time_interval, 0)
