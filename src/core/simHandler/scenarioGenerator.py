...
TOD = np.arange(
    max(TOD_START - WARM_UP_PERIOD, 0),
    min(24, TOD_END + COOL_DOWN_PERIOD),
    DEMAND_INTERVAL,
)
...
def create_scenario(path_trips, path_routes, path_demand=PATH_DEMAND):
...
def trip_validator(path_trips, path_routes):
    ...
    subprocess.run(
        "python "
        + SUMO_PATH
        + "/tools/purgatory/route2trips.py "
        + path_routes
        + " > "
        + path_trips,
        shell=True,
